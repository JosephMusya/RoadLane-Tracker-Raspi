...
class Lines():
    ...
    def project_on_road_debug(self, image_input, debug = True):
        ...
        if self.detected_first & self.detected:
            ...
            result1 = cv2.addWeighted(result, 1, np.vstack((filler,newwarp)), 0.3, 0)
            ...
            cv2.putText(result2,img_text, (15,15), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,0,0),2)
            cv2.putText(result2,img_text2,(15,40), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,0,0),2)
            
            return result1,result2,self.offset,self.left_curverad,self.right_curverad
        ...
        # if lanes were not detected output source image
            ...
            return return_image,return_image,0,0,0
...
cap = cv2.VideoCapture(r'C:\Users\josep\Desktop\MyCode\Advanced-Lanes\challenge_video.mp4')
print("Started")
def main(debug=True):
    while True:
        _,img = cap.read(0)
        
        result1,result2,offset,left_c,right_c = lines.project_on_road_debug(img)
        print("Offset: ",offset)
        print("Right Curvature: ", right_c)
        print("Left Curvature: ", left_c)
        print()
        if debug:
            print("resized")
            cv2.resize(result1, (500,250))
            cv2.resize(result2, (500,250))
            
            cv2.imshow("Rpi lane detection",result1)
            cv2.imshow("Rpi lane detection",result2)
